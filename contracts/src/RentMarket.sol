...
contract RentMarket is EIP712 {
  ...
  constructor() EIP712("RentMarket", "1") { }
  ...
  function borrow(RentoutOrder calldata order, bytes calldata makerSignature) external payable {
    ...
    address recoveredSigner = ECDSA.recover(hash, makerSignature);
    if (order.maker != recoveredSigner) revert("Invalid signature");
    if (canceledOrders[hash]) revert("Order canceled");
    IERC721(order.nft_ca).safeTransferFrom(order.maker, msg.sender, order.token_id);
    orders[hash] = BorrowOrder({ taker: msg.sender, start_time: block.timestamp, rentinfo: order });
  }
  ...
  function cancelOrder(RentoutOrder calldata order, bytes calldata makerSignatre) external {
    checkSignature(order, makerSignatre);
    // todo: 取消订单的成本

    canceledOrders[_hashTypedDataV4(orderHash(order))] = true;
    emit OrderCanceled(msg.sender, _hashTypedDataV4(orderHash(order)));
  }

  function checkSignature(RentoutOrder calldata order, bytes calldata makerSignatre) internal view {
    bytes32 hash = _hashTypedDataV4(orderHash(order));
    address recoveredSigner = ECDSA.recover(hash, makerSignatre);
    if (order.maker != recoveredSigner) revert("Invalid signature");
  }
